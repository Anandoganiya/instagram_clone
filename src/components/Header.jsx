    ...
    <header className='w-full shadow-md bg-white'>
      <nav className='md:w-3/4 w-full mx-auto p-3 flex justify-between'>
        <div className='w-[7rem]'>
          <Link to={`/`}> 
            <img className='w-full' src="/images/users/logo.png" alt="instagram logo" />
          </Link>
        </div>
        {
          user?(
            ...
            <Link to={`/p/${userInfo.user.username}`}>
              <img className='rounded-2xl object-cover w-8 text-black-light cursor-pointer' src={`/images/avatars/${userInfo.user.username}.jpg`} alt="profile" />
            </Link>
              </div>
          ):(
              <div className='flex space-x-4'>
